...
public class QuickDotProgressBarView  extends View {
     ...
     * {@link QuickDotProgressBarView#RIGHT_DIRECTION} and {@link QuickDotProgressBarView#LEFT_DIRECTION}
     * */
    private int animationDirection;

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public QuickDotProgressBarView(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        initializeAttributes(attrs);
        init();
    }

    public QuickDotProgressBarView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initializeAttributes(attrs);
        init();
    }

    public QuickDotProgressBarView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initializeAttributes(attrs);
        init();
    }

    public QuickDotProgressBarView(Context context) {
        super(context);
    ...
    private void initializeAttributes(AttributeSet attrs) {
        if (attrs != null) {
            TypedArray a = getContext().getTheme().obtainStyledAttributes(
                    attrs,
                    R.styleable.QuickDotProgressBarView,
                    0, 0);

            try {
                setDotAmount(a.getInteger(R.styleable.QuickDotProgressBarView_amount, 5));
                setAnimationTime(animationTime = a.getInteger(
                        R.styleable.QuickDotProgressBarView_duration,
                        getResources().getInteger(android.R.integer.config_mediumAnimTime)
                ));
                setStartColor(
                        a.getInteger(
                                R.styleable.QuickDotProgressBarView_startColor,
                                ContextCompat.getColor(getContext(), R.color.light_blue)
                        )
                );
                setEndColor(
                        a.getInteger(
                                R.styleable.QuickDotProgressBarView_endColor,
                                ContextCompat.getColor(getContext(), R.color.light_blue)
                        )
                );
                setAnimationDirection(a.getInt(R.styleable.QuickDotProgressBarView_animationDirection, 1));
            } finally {
                a.recycle();
            }
            ...
            setStartColor(ContextCompat.getColor(getContext(), R.color.blue_default));
            setEndColor(ContextCompat.getColor(getContext(), R.color.light_blue));
